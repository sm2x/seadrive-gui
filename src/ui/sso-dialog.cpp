    ...
    : QDialog(parent)
{
    ...
    computer_name_ = gui->settingsManager()->getComputerName();
...
{
    QString server_addr =
        gui->readPreconfigureEntry(kPreconfigureSSOLoginUrl)
            .toString()
            .trimmed();
    ...
    if (url.isEmpty())
    {
        url = "https://file.alibaba-inc.com";
    }

    gui->settingsManager()->setLastSSOUrl(url.toString());
    server_addr_ = url;
    getSSOLink();
}
...
bool SSODialog::getSSOLoginUrl(const QString &last_url, QUrl *url_out)
{
    ...
    // while (true) {
    //     bool ok;
    //     // TODO: uncomplete this function func write by sun
    //     server_addr =
    //         gui->getText(this,
    //                             tr("Single Sign On"),
...
void SSODialog::onFetchAccountInfoSuccess(const AccountInfo &info)
{
    ...
    gui->trayIcon()->showMessage(getBrand(), buf);
    accept();
}
